...
def parse_args():
    ...
    group.add_argument(
        ...
        'Output of ./cooccur')
    group.add_argument(
        'ppmi', type=str, help='Path to ppmi.npz containing a sparse (COO) '
        'representation of the PPMI matrix in numpy archive format. '
        'Output of ./ppmi')
...
def get_train_data(args):
    ...
    ppmi_npz = np.load(args.ppmi)
    assert np.all(row == ppmi_npz['row'])
    assert np.all(col == ppmi_npz['col'])
    ppmi = ppmi_npz['data']  # TODO the values seem strange
    ...
    ppmi = mx.nd.array(ppmi, dtype='float32')
    ...
    if ppmi.shape != counts.shape:
        ppmi = mx.nd.concatenate([ppmi, ppmi])
        assert ppmi.shape == counts.shape
    return vocab, row, col, counts, ppmi


# * Gluon Block definition
class GloSG(nlp.model.train.EmbeddingModel, mx.gluon.HybridBlock):
    def __init__(self, token_to_idx, output_dim, x_max, alpha,
                 weight_initializer=None,
                 bias_initializer=mx.initializer.Zero(), sparse_grad=True,
                 dtype='float32', **kwargs):
        super(GloSG, self).__init__(**kwargs)
    ...
    def hybrid_forward(self, F, row, col, weight, ppmi):
        ...
        dot = F.batch_dot(emb_in.expand_dims(1), emb_out.expand_dims(2))
        tmp = dot.squeeze() - ppmi.squeeze()
        loss = weight.squeeze() * F.square(tmp)
        return loss
...
class EmbeddingARS(mx.optimizer.Optimizer):
    def __init__(self, trust=0.0001, eps=1e-5, **kwargs):
        super(EmbeddingARS, self).__init__(**kwargs)
    ...
    def update(self, index, weight, grad, state):
        ...
        assert wd == 0, 'Weight decay is not supported'
        ...
        if is_sparse:
            ...
            mx.nd.contrib.embedding_ars_update(weight, grad, out=weight, lr=lr,
...
def train(args):
    """Training helper."""
    vocab, row, col, counts, ppmi = get_train_data(args)
    model = GloSG(token_to_idx=vocab.token_to_idx, output_dim=args.emsize,
                  x_max=args.x_max, alpha=args.alpha,
    ...
    for epoch in range(args.epochs):
        ...
        for i in range(num_batches):
            ...
            batch_ppmi = ppmi[batch_indices].as_in_context(ctx)
            with mx.autograd.record():
                loss = model(batch_row, batch_col, batch_counts, batch_ppmi)
            loss.backward()
