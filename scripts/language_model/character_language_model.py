...
This example shows how to build a character-level language model on WikiText-2 with Gluon NLP Toolkit.
...
parser = argparse.ArgumentParser(
    description='MXNet Autograd RNN/LSTM '
    'Language Model on Wikitext-2 / Wikitext-103.')
parser.add_argument('--model', type=str, default='lstm',
                    help='type of recurrent net '
                    '(rnn_tanh, rnn_relu, lstm, gru)')
parser.add_argument('--dataset', type=str, default='Wikitext-2',
                    help='Training dataset (Wikitext-2, Wikitext-103)')
parser.add_argument('--lower', action='store_true',
                    help='Lower all characters.')
parser.add_argument('--byte-vocab', action='store_true',
                    help='Use all possible 256 byte values for vocab '
                    'independently of which appear in the dataset.')
...
parser.add_argument('--dropout_h', type=float, default=0.01,
                    help='dropout applied to hidden layer (0 = no dropout)')
parser.add_argument('--dropout_i', type=float, default=0.01,
                    help='dropout applied to input layer (0 = no dropout)')
parser.add_argument('--dropout_e', type=float, default=0.0,
                    help='dropout applied to embedding layer (0 = no dropout)')
parser.add_argument(
    '--weight_dropout', type=float, default=0.5,
...
with open(args.save + '-args', 'w') as f:
    f.write(str(args))
...
def print_time(task):
    ...
    print('Starting to {}'.format(task))
    yield
    print('Finished to {} in {} seconds'.format(task,
...
if args.dataset.lower() == 'wikitext-2':
    dataset_ = nlp.data.WikiText2Raw
elif args.dataset.lower() == 'wikitext-103':
    dataset_ = nlp.data.WikiText103Raw
else:
    print(args.dataset, 'is unsupported.')
    sys.exit(1)

with print_time('load dataset'):
    if args.lower:

        def tokenizer(s):
            return list(s.lower().encode('utf-8'))
    ...
    train_dataset, val_dataset, test_dataset = [
        dataset_(segment=segment, skip_empty=False, bos=None, eos=None,
                 tokenizer=tokenizer) for segment in ['train', 'val', 'test']
    ]
    # Construct word level dataset to compute word level perplexity
    train_dataset_wordlevel = dataset_(segment='train', skip_empty=False,
                                       bos=None, eos='<eos>',
                                       tokenizer=word_tokenizer)

with print_time('build vocab'):
    if not args.byte_vocab:
        vocab = nlp.Vocab(
            counter=nlp.data.Counter(train_dataset[0]), padding_token=None,
            bos_token=None, eos_token=None, unknown_token=0)
    else:
        vocab = nlp.Vocab(
            counter=nlp.data.Counter(range(2**8)), padding_token=None,
            bos_token=None, eos_token=None, unknown_token=None)
    word_vocab = nlp.Vocab(
        counter=nlp.data.Counter(train_dataset_wordlevel[0]),
        padding_token=None, bos_token=None, eos_token=None)
    vocab.to_json(args.save + '-vocab.json')
    word_vocab.to_json(args.save + '-vocab.json')
    print('Vocabulary ', vocab)
    print('Word vocabulary ', vocab)

with print_time('build batches'):
...
if args.weight_dropout > 0:
    print('Use AWDRNN')
    model = nlp.model.language_model.AWDRNN(
        args.model, len(vocab), args.emsize, args.nhid, args.nlayers,
...
if args.optimizer == 'sgd':
    trainer_params = {'learning_rate': args.lr, 'momentum': 0, 'wd': args.wd}
...
def criterion(output, target, encoder_hs, dropped_encoder_hs):
    ...
    if args.alpha:
        dropped_means = [
            args.alpha * dropped_encoder_h.__pow__(2).mean()
            for dropped_encoder_h in dropped_encoder_hs[-1:]
        ]
        l = l + mx.nd.add_n(*dropped_means)
    if args.beta:
        means = [
            args.beta * (encoder_h[1:] - encoder_h[:-1]).__pow__(2).mean()
...
def train():
    ...
    for epoch in range(args.epochs):
        ...
        hiddens = [
            model.begin_state(args.batch_size // len(context),
                              func=mx.nd.zeros, ctx=ctx) for ctx in context
        ]
        batch_i, i = 0, 0
        while i < len(train_data) - 1 - 1:
            bptt = args.bptt if mx.nd.random.uniform().asscalar(
            ...
            data_list = gluon.utils.split_and_load(data, context, batch_axis=1,
                                                   even_split=True)
            ...
            with autograd.record():
                for j, (X, y, h) in enumerate(
                        zip(data_list, target_list, hiddens)):
            ...
            if batch_i % args.log_interval == 0 and batch_i > 0:
                ...
                print('[Epoch %d Batch %d/%d] loss %.2f, ppl %.2f, '
                      ...
                      (epoch, batch_i, len(train_data) // args.bptt, cur_L,
                       math.exp(cur_L),
                       math.exp(cur_L * len(vocab) / len(word_vocab)),
        ...
        print('[Epoch %d] throughput %.2f samples/s' %
              (epoch, (args.batch_size * len(train_data)) /
               (time.time() - start_epoch_time)))
        val_L = evaluate(val_data, val_batch_size, context[0])
        print('[Epoch %d] time cost %.2fs, valid loss %.2f, '
              'valid ppl %.2f, valid word ppl %.2f' %
              (epoch, time.time() - start_epoch_time, val_L, math.exp(val_L),
               math.exp(val_L * len(vocab) / len(word_vocab))))
        ...
        if val_L < best_val:
            ...
            print('test loss %.2f, test ppl %.2f, test word ppl %.2f' %
                  (test_L, math.exp(test_L),
                   math.exp(test_L * len(vocab) / len(word_vocab))))
        else:
            update_lr_epoch += 1
...
if __name__ == '__main__':
    ...
    print('Best validation loss %.2f, val ppl %.2f' %
          (final_val_L, math.exp(final_val_L),
           math.exp(final_val_L,
                    *len(vocab) / len(word_vocab))))
    print('Best test loss %.2f, test ppl %.2f' %
          (final_test_L, math.exp(final_test_L),
           math.exp(final_test_L * len(vocab) / len(word_vocab))))
